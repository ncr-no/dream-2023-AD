...
func (c *WateringController) Store(ctx *gin.Context) {
	...
	if garden.UserID == user.ID {
		ctx.Redirect(http.StatusFound, fmt.Sprintf("/gardens/%s/water", garden.HashId(c.hid)))
		return
	}
	...
	ctx.Redirect(http.StatusFound, fmt.Sprintf("/gardens/%s/water#request-%d", garden.HashId(c.hid), request.ID))
}

func (c *WateringController) ApproveRequest(ctx *gin.Context) {
	garden, err := findGarden(ctx, c.hid, c.db)
	if err != nil {
		_ = ctx.AbortWithError(http.StatusInternalServerError, err)
		return
