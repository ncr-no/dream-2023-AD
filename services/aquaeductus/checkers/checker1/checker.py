...
class WeatherReportChecker(checkerlib.BaseChecker):
    ...
    def check_flag(self, tick: int) -> Tuple[CheckResult, str]:
        ...
        resp = requests.post(f'{self.baseurl}/registration', data={'username': user1, 'password': pw1},
        ...
        resp = requests.post(f'{self.baseurl}/registration', data={'username': user2, 'password': pw2},
                             allow_redirects=False,
                             timeout=self.timeout)
        if resp.status_code != 302:
