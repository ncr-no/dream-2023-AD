...
class GardenChecker(checkerlib.BaseChecker):
    ...
    def place_flag(self, tick: int) -> Tuple[CheckResult, str]:
        ...
        resp = requests.post(f'{self.baseurl}/registration', data={'username': user, 'password': pw}, allow_redirects=False,
    ...
    def check_flag(self, tick: int) -> Tuple[CheckResult, str]:
        ...
        resp = requests.get(f'{self.baseurl}/gardens/{garden_id}/water', cookies=cookies, timeout=self.timeout)
        if resp.status_code != 200:
            logging.warning('Garden Watering - Got incorrect status code %s', resp.status_code)
            return CheckResult.FAULTY, 'cannot find garden'

        resp = requests.post(f'{self.baseurl}/gardens/{garden_id}/water', data={
        ...
        resp = requests.post(f'{self.baseurl}/gardens/{garden_id}/water/{request_id}', cookies=owner_cookies,
                             allow_redirects=False, timeout=self.timeout)
        if resp.status_code != 302:
        ...
        if resp.headers['Location'] != f'/gardens/{garden_id}/water':
