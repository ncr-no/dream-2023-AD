...
void add_treasure(void) {
	...
	if (strchr(name, '.') != NULL || strchr(name, '/') != NULL) {
		printf("Unallowed chars in location name!\n");
		return;
	}
	...
	if (ret == -1) {
		...
		printf("There is already a coin at this location!\n");
		fflush(stdout);
	} else {
		printf("Great! We'll safely store this information for you! :)\n");
		fflush(stdout);
	}
}
...
void view_treasure(void) {
	...
	if (tr == NULL) {
		printf("No Coin found at this location!\n");
		return;
	}
...
void modify_treasure(void) {
	...
	printf("Push feature in next release.\n");
}
